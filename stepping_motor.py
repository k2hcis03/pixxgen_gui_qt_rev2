...
class SteppingMotor:
    def __init__(self, dev_gpio, gpio_i2c_datas, gpio_i2c_parsing_data, logging):
        ...
        self.motor1_dir = self.motor2_dir = self.motor3_dir = self.motor4_dir = self.CW
    ...
    def st_motor_start(self, st_motor_number, start, speed, count):
        if st_motor_number == 1:
            if start == True:
                ...
                if count < 0:
                    self.st_motor_dir(1, self.CCW)
                else:
                    self.st_motor_dir(1, self.CW)
                if self.motor1_dir == self.CW and self.gpio_i2c_parsing_data['step1_enc1'][2] == 1:
                    return
                if self.motor1_dir == self.CCW and self.gpio_i2c_parsing_data['step1_enc3'][2] == 1:
                    return
            ...
            if start == True:
                ...
                if count < 0:
                    self.st_motor_dir(2, self.CCW)
                else:
                    self.st_motor_dir(2, self.CW)
                if self.motor2_dir == self.CW and self.gpio_i2c_parsing_data['step2_enc1'][2] == 1:
                    return
                if self.motor2_dir == self.CCW and self.gpio_i2c_parsing_data['step2_enc3'][2] == 1:
                    return
                data.st_motor_speed = speed
                data.st_motor_count = abs(count)
                fcntl.ioctl(self.dev_gpio, SET_DATA, data)
            ...
            if start == True:
                ...
                if count < 0:
                    self.st_motor_dir(3, self.CCW)
                else:
                    self.st_motor_dir(3, self.CW)
                if self.motor3_dir == self.CW and self.gpio_i2c_parsing_data['step3_enc1'][2] == 1:
                    return
                if self.motor3_dir == self.CCW and self.gpio_i2c_parsing_data['step3_enc2'][2] == 1:
                    return                
                data.st_motor_speed = speed
                data.st_motor_count = abs(count)
                fcntl.ioctl(self.dev_gpio, SET_DATA, data)
                ...
                if self.motor3_dir != self.STOP:
                    self.motor3_dir = self.STOP
                    fcntl.ioctl(self.dev_gpio, SET_DATA, data)   
            ...
            if start == True:
                ...
                if count < 0:
                    self.st_motor_dir(4, self.CCW)
                else:
                    self.st_motor_dir(4, self.CW)
                if self.motor4_dir == self.CW and self.gpio_i2c_parsing_data['step4_enc1'][2] == 1:
                    return
                if self.motor4_dir == self.CCW and self.gpio_i2c_parsing_data['step4_enc2'][2] == 1:
                    return                
                data.st_motor_speed = speed
                data.st_motor_count = abs(count)
                fcntl.ioctl(self.dev_gpio, SET_DATA, data)
                ...
                if self.motor4_dir != self.STOP:
                    self.motor4_dir = self.STOP
                    fcntl.ioctl(self.dev_gpio, SET_DATA, data)  
